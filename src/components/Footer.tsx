...
export const Footer: React.FC<FooterProps> = ({ onSelectTurma, setActiveTab }) => {
  ...
  const link: React.CSSProperties = {
    fontSize: 13, color: '#81a882', background: 'none', border: 'none',
    cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit',
    padding: 0, lineHeight: 2, transition: 'color 0.15s',
  };

  return (
    <footer style={{ background: '#1b2e1c', borderTop: '2px solid #2d4a2e', marginTop: 64 }}>
      <div className="container" style={{ padding: '52px 24px 32px' }}>
        <div className="footer-grid">
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#81a882', lineHeight: 1.7, maxWidth: 300 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {TURMAS_LIST.map(t => (
                <button key={t.id} style={link}
                  ...
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = '#81a882'}>
                  {t.name}
                  <span style={{ color: '#3d5140', marginLeft: 4 }}>({t.code})</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <button onClick={scrollTop}
                style={{ ...link, color: '#81c784', fontWeight: 700, marginTop: 8, display: 'flex', alignItems: 'center', gap: 4 }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = '#a5d6a7'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = '#81c784'}>
                Voltar ao topo
        ...
        <div style={{
          ...
          borderTop: '1px solid #2d4a2e',
          display: 'flex', flexWrap: 'wrap',
          alignItems: 'center', justifyContent: 'space-between', gap: 8
        }}>
          <span style={{ fontSize: 12, color: '#3d5140' }}>
            © 2026 Instituto Providência • Todos os direitos reservados.
          </span>
          <div style={{ display: 'flex', gap: 12, fontSize: 12, color: '#3d5140' }}>
